...
public class IntentHandler {
    ...
    private void handleDownloadIntent() {
        ...
        if (friend == null) {
            ...
            new SyncManager(service).getAndPollAllFriends();
            return;
        }
        ...
        if (status == Video.IncomingVideoStatus.DOWNLOADED) {
            ...
            if (!TbmApplication.getInstance().isForeground() || Convenience.screenIsLockedOrOff(service)) {
                NotificationAlertManager.alert(service, friend, videoId);
            }
            service.removeDownloadId(videoId);
        }
