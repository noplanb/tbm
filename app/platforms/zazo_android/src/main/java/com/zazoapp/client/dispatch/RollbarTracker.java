...
public class RollbarTracker implements ErrorTracker {
    ...
    private void setUpUser() {
        ...
        try {
            person.put("id", "-");
            person.put("username", "-");
            person.put("phone_number", "-");
            person.put("host", Config.getServerHost());
            person.put("android_id", Settings.Secure.getString(TbmApplication.getInstance().getContentResolver(), Settings.Secure.ANDROID_ID));
            TelephonyManager tm = (TelephonyManager) TbmApplication.getInstance().getSystemService(Context.TELEPHONY_SERVICE);
            if (tm != null) {
                person.put("device_id", tm.getDeviceId());
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
